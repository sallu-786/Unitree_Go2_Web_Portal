...
logger = logging.getLogger("ros_mcp.websocket")
...
class WebSocketManager:
    ...
    def set_ip(self, ip: str, port: int):
        ...
        self.ip = ip
        self.port = port
        logger.info(f"IP set to {self.ip}:{self.port}")
        ...
        with self.lock:
            if self.ws is None or not self.ws.connected:
                try:
                    url = f"ws://{self.ip}:{self.port}"
                    self.ws = websocket.create_connection(url, timeout=self.default_timeout)
                    logger.info(f"Connected ({self.default_timeout}s timeout)")
                    return None  # no error
                except Exception as e:
                    error_msg = f"[WebSocket] Connection error: {e}"
        ...
        with self.lock:
            ...
            if self.ws:
                try:
                    ...
                    logger.warning(f"Receive error or timeout: {e}")
                    self.close()
                    return None
            return None

...
class WebRTCBackend:
    ...
    def initialize(self):
        ...
        led_client = WebRTCLed(self.conn)
...
class WebRTCVideo:
    ...
    def run_async_loop(self, loop: asyncio.AbstractEventLoop, setup: Callable):
...
class WebRTCLed:
    ...
    async def set_led(self, color: str = "white", brightness: int = 5, mode: str = "steady"):
        """
        color: one of VUI_COLOR (red, green, blue, etc.)
        brightness: 0–10 (safe range)
        mode: "steady" or "blink"
        """

        payload = {
            "api_id": 1001,  # VUI_API_ID_SETSWITCH
            "parameter": {
                "color": color,
        ...
        response = await self.conn.datachannel.pub_sub.publish_request_new(
            RTC_TOPIC["VUI"], payload
        )

        # Debug (keep this while testing)
        print("LED response:", response)

    async def set_brightness(self, brightness: int):
        payload = {
            "api_id": 1005,  # official VUI brightness API
            "parameter": {
                "value": brightness
            }
        }
        ...
        print("Brightness response:", response)

    # 🔴 Presets
    async def red(self):
        await self.set_led(color=VUI_COLOR.RED)

    async def green(self):
        await self.set_led(color=VUI_COLOR.GREEN)

    async def blue(self):
        await self.set_led(color=VUI_COLOR.BLUE)

    async def white(self):
        await self.set_led(color=VUI_COLOR.WHITE)

    async def purple(self):
        await self.set_led(color=VUI_COLOR.PURPLE)

    async def yellow(self):
        await self.set_led(color=VUI_COLOR.YELLOW)

    async def cyan(self):
        await self.set_led(color=VUI_COLOR.CYAN)

    async def off(self):
        await self.set_led(color="black", brightness=0)

    async def blink_red(self):
        await self.set_led(color=VUI_COLOR.RED, mode="blink")

    async def dim_blue(self):
        await self.set_led(color=VUI_COLOR.BLUE, brightness=2)